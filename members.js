...
function renderNav(){ 
  $('nav').innerHTML=user ? `<button data-go="dashboard">Dashboard</button><button data-go="course">Curso</button><button data-go="profile">Meu perfil</button>${profile?.role==='admin'?'<button data-go="admin">Admin</button>':''}<button id="logout">Sair</button>` : ''; 
}

function authMode(){ 
  $('auth-title').textContent=signup?'Criar conta':'Entrar na tua conta';$('auth-subtitle').textContent=signup?'Cria a tua conta para acompanhar o teu progresso.':'Continua a aprender no teu ritmo.'; 
  $('name-field').style.display=signup?'block':'none';$('confirm-field').style.display=signup?'block':'none'; 
  $('auth-submit').textContent=signup?'Criar conta':'Entrar';$('toggle-auth').textContent=signup?'Já tenho uma conta':'Ainda não tenho conta'; 
  $('forgot').style.display=signup?'none':'inline-block'; 
}
...
function openLesson(l){
  selectedLesson=l; 
  $('lesson-module').textContent=`Aula ${l.number||''} · ${l.modules?.title||''}`; 
  $('lesson-title').textContent=l.title; 
  $('lesson-description').textContent=l.description\vert{}\vert{}'';$('complete-btn').textContent=progress.find(p=>p.lesson_id===l.id)?.completed?'Concluída ✓':'Marcar como concluída'; 
  window.membersNavigate('course'); // Mantém na rota correta
}

function escapeHtml(s=''){ return s.replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c])); }

// Funções de controlo de sessão atualizadas para o novo Router
...
$('toggle-auth').onclick=()=>{ signup=!signup; authMode(); message(''); }; 
...
// O redirecionamento por cliques nos botões [data-go] já é tratado automaticamente pelo members-routes.js
...
// Listener para o botão de sair se existir no DOM
document.addEventListener('click', (e) => {
...
const observer=new MutationObserver(()=>{
  if($('profile-view')?.classList.contains('active')&&profile){
    $('profile-name').value=profile.name\vert{}\vert{}'';$('profile-email').value=user.email;
    $('profile-avatar').value=profile.avatar_url\vert{}\vert{}'';$('created-at').textContent=profile.created_at?`Conta criada em ${new Date(profile.created_at).toLocaleDateString('pt-PT')}`:'';
  }
});
